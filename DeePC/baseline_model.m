...
Ts    = 0.25;          % 采样周期，1 秒采一次
TsNs = Ts * 1e9;
Nstep = 400;         % 一共要发送多少个样点（可根据需要改）

u = zeros(3,Nstep);     % control input log
y = zeros(3,Nstep);     % output log
...
% 检查轨迹长度是否足够
if length(phi_b_deg_trajectory) < (Nstep) || length(gamma_g_deg_trajectory) < (Nstep)
