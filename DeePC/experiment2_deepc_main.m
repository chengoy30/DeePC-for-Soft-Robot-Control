...
% i_data = 2;
load(['_data\trajectory_data_collection\ioput','.mat']);  % 需要包含 ud, yd (及可能的 ed)
...
%% === 构造圆形参考轨迹（144x3），沿 φ 扫描 ===
num_points = 144; 
trajectory = zeros(num_points, 3);
for i = 0:num_points-1
    phi = (2 * pi) / num_points * i;        % 弧度
    [x_r, y_r, z_r] = constantCurvature(theta, phi, L_arm);
    trajectory(i+1, :) = [x_r, y_r, -z_r+2];    % 保持你原来的 -z_r
end

window_size = N;
need_len = total_step + window_size;
rep = ceil((need_len + 100)/num_points);   % 冗余
Yref_ext = repmat(trajectory, rep, 1);
